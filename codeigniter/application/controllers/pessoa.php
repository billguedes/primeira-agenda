...
class Pessoa extends CI_Controller {
    ...
    public function lista() {
        ...
        $lista = $this->pessoa_model->listaPorNome($filtro, $limite, $inicio);        
        ...
        if (isset($lista)) {
            ...
            if (isset($filtro)) {
                ...
                $total = $this->pessoa_model->totalRegistro();
            }            
        } else {
            $lista = array();
            $sucesso = FALSE;
    ...
    private function listaCodifica($sucesso, $lista = NULL, $total = NULL) {    
        $mensagem = $this->pessoa_model->mensagemErro();
        $dados['pessoa'] = $this->codifica($sucesso, $mensagem, $lista, $total);
    ...
    private function visaoJson($dados) {
        $this->load->view('pessoa/json', $dados);
    }
    
    private function visaoXml($dados) {
        $this->load->view('pessoa/xml', $dados);
    }
    ...
    public function adiciona() {
        $pessoas = $this->capturaEntrada();
        $dados = $this->decodifica($pessoas);
        
        $sucesso = $this->pessoa_model->adiciona($dados);
    ...
    public function atualiza() {
        $pessoa = $this->capturaEntrada();
        $dados = $this->decodifica($pessoa);
        $codigo = $dados->codigo;
        
        $sucesso = $this->pessoa_model->atualiza($dados, $codigo);
        $dados = $this->listaCodifica($sucesso);
    ...
    public function exclui() {
        ...
        $sucesso = $this->pessoa_model->exclui($codigo);
        $dados = $this->listaCodifica($sucesso);
    ...
    private function decodificaXml($dados) {
        $dados = (object)(array)$dados->pessoa;
